...
const showcases = [
  ...
  {
    src: "https://images.unsplash.com/photo-1532096316719-216dc7b11d9f?q=80&w=1968&auto=format&fit=crop",
    title: "Vintage",
          ...
          {showcases.map((item, index) => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 p-6 md:p-8 text-white transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300 z-10">
                <span className="text-primary font-serif italic text-lg mb-1 block">{item.title}</span>
                <h3 className="text-xl md:text-2xl font-bold mb-2">{item.desc}</h3>
              </div>
            </motion.div>
          ))}
        </div>
